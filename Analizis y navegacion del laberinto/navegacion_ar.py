...
import cv2                  # OpenCV para procesamiento de video, dibujos AR y detección de ArUcos.
import numpy as np          # NumPy para trigonometría, vectores, matrices y distancias (hypot, linalg.norm).
import time                 # Para el cronómetro del recorrido y la lógica de atascos.
import rclpy                # Para mantener activo el nodo de ROS 2 durante el bucle de video.
import os                   # Para crear carpetas y guardar los archivos de métricas.

# =====================================================================
# FUNCIÓN PRINCIPAL: INICIAR REALIDAD AUMENTADA Y CONTROL
# =====================================================================
def iniciar_realidad_aumentada(camino_logico, puerta_ent, puerta_sal, color_bgr_ignorado, cells_x, cells_y, ros_node, datos_resultado, trial_path):
    # Variables de configuración del lienzo virtual
    tamano_warp = 800       # El tablero físico se deformará digitalmente a un cuadrado perfecto de 800x800 px.
    margen_corte = 7        # Píxeles de margen para ignorar la cinta negra del borde del laberinto físico.
    
    # Cálculos para adaptar la cuadrícula teórica a la cuadrícula de píxeles
    ancho_real = tamano_warp - (2 * margen_corte)
    alto_real = tamano_warp - (2 * margen_corte)
    paso_x = ancho_real / cells_x # Cuántos píxeles de ancho tiene cada celda
    paso_y = alto_real / cells_y  # Cuántos píxeles de alto tiene cada celda
    
    # -----------------------------------------------------------------
    # TRADUCTOR DE COORDENADAS MATEMÁTICAS A PÍXELES
    # -----------------------------------------------------------------
    def coord(n):
        # Si es la meta o inicio, usa las variables especiales de las puertas.
        if n == 'START': p = puerta_ent
        elif n == 'END': p = puerta_sal
        else: 
            # Si es un nodo normal, calcula el centro de esa celda en píxeles.
            return (int((n[0] + 0.5) * paso_x) + margen_corte, int((n[1] + 0.5) * paso_y) + margen_corte)
        
        # Lógica especial para las puertas: las pega exactamente al borde del lienzo de 800x800 según su punto cardinal.
    ...
    # Convierte toda la ruta teórica de A* a píxeles.
    puntos = [coord(n) for n in camino_logico]
    nodos_clave = [] # Aquí guardaremos solo los vértices (donde hay que doblar).
    
    # --- DETECCIÓN DEL PRIMER GIRO ---
    # Crea vectores para el primer movimiento y el segundo movimiento.
    v_in = (puntos[1][0] - puntos[0][0], puntos[1][1] - puntos[0][1])
    v_out = (puntos[2][0] - puntos[1][0], puntos[2][1] - puntos[1][1])
    # El Producto Cruz detecta giros. Si es 0, va recto. Si es positivo, gira derecha. Si es negativo, izquierda.
    ...
    # --- DETECCIÓN DE GIROS EN EL RESTO DEL LABERINTO ---
    ...
    nodos_clave.append({'pt': puntos[-2], 'tipo': 'FIN', 'cmd': 'STOP'}) # El último nodo siempre es la parada.

    # Detecta qué nodo de la lista es el primer objetivo real hacia el que debe mirar el auto.
    indice_objetivo = 0
    if nodos_clave[0]['tipo'] == 'INICIO_RECTO':
        for i in range(1, len(nodos_clave)):
            if nodos_clave[i]['tipo'] in ['GIRO', 'FIN']:
                indice_objetivo = i; break

    # -----------------------------------------------------------------
    # GENERACIÓN DE PISTAS RECTAS PARA ENTRADA Y SALIDA
    # -----------------------------------------------------------------
    longitud_guia = min(paso_x, paso_y) * 0.5 # Media celda de distancia proyectada hacia afuera.
    
    # Entrada: Crea un vector desde el borde hacia adentro, lo normaliza (longitud 1) y lo proyecta hacia afuera.
    ...
    # Salida: Lo mismo pero proyectando el último vector hacia el exterior del laberinto.
    ...
    # --- GUARDADO INICIAL DE DATOS EXPERIMENTALES ---
    ...
    # =====================================================================
    # INICIO DE LA CÁMARA EN TIEMPO REAL
    # =====================================================================
    cap = cv2.VideoCapture(2) # Abre la cámara USB.
    print("\n🟢 PRUEBA DE MOTOR: SEGUIDOR CONTINUO SIN DETENCIONES 🟢")
    print("Manten la camara quieta. Observa el acoplamiento logico-fisico.")
    
    # Banderas y variables de estado para el ciclo AR
    tiempo_inicio = time.time()
    mapeo_bloqueado = False
    navegacion_iniciada = False
    nodo_alcanzado = False  
    tiempo_fase_asoc = 0
    asociacion_calculada, asociacion_hecha, reemplazo_hecho = False, False, False
    tablero_cnt_est, M_inv_est, M_est = None, None, None
    arucos_maze = {}      

    tiempo_inicio_recorrido, tiempo_transcurrido = 0.0, 0.0
    cronometro_corriendo, tiempo_guardado = False, False
    video_writer, video_count = None, 0
    ...
    ultimo_comando_enviado = None # Filtro Anti-Spam

    # =====================================================================
    # BUCLE PRINCIPAL (Se ejecuta ~30 veces por segundo)
    # =====================================================================
    while True:
        ret, frame = cap.read()
        if not ret: break
        h_frame, w_frame = frame.shape[:2]
        frame_limpio = frame.copy() # Copia limpia para que ArUco la lea sin los dibujos de encima
        tiempo_actual = time.time()
        
        comando_ros2 = "STOP"
        bloquear_envio = False # Bandera de "Silencio de Radio"

        if cronometro_corriendo:
            tiempo_transcurrido = time.time() - tiempo_inicio_recorrido

        # --- MAPEO DEL TABLERO (Solo los primeros 2 segundos) ---
        if not mapeo_bloqueado:
            # Filtros para encontrar el rectángulo exterior del laberinto.
            ...
            if tablero_temp is not None:
                ...
                # Obtiene las matrices de homografía (M_est para pasar cámara a 2D plano, M_inv_est para AR)
                M_inv_est = cv2.getPerspectiveTransform(dst, rect)
                M_est = cv2.getPerspectiveTransform(rect, dst)
                
            # Pasados 2 segundos, "congela" la detección del tablero para que no vibre.
            if tiempo_actual - tiempo_inicio >= 2.0 and tablero_cnt_est is not None:
                mapeo_bloqueado = True

        cx_auto, cy_auto, pt_sensor_central = None, None, None
        ...
        if ids is not None:
            for i, aruco_id in enumerate(ids):
                id_val = aruco_id[0]
                # Calcula el centro exacto del marcador promediando sus 4 esquinas
                cx = int(np.mean(corners[i][0][:, 0]))
                cy = int(np.mean(corners[i][0][:, 1]))
                
                # SI ENCUENTRA EL ROBOT (ID 100)
                if id_val == 100:
                    cx_auto, cy_auto = cx, cy
                    
                    longitud_extension_t = 12 # Proyecta el sensor 12 píxeles adelante del robot
                    radio_puntos = 4          

                    # Obtiene las 4 esquinas para determinar la orientación de la "trompa"
                ...
                # SI ENCUENTRA NODOS DEL LABERINTO (ID 1 a 25) y aún estamos calibrando
                elif 1 <= id_val <= 25 and not asociacion_calculada:
                    if M_est is not None:
                        # Pasa la coordenada de la cámara al plano 2D del tablero y la guarda
        ...
        txt_fase, txt_tiempo, txt_instrucciones = "", "", ""
        txt_sensor_central, txt_sensor_lado, txt_completado, txt_movimiento = "", "", "", ""

        if tablero_cnt_est is not None:
            lienzo_ar_dinamico = np.zeros((tamano_warp, tamano_warp, 4), dtype=np.uint8) # Capa transparente
            
            # =================================================================
            # FASES DE CALIBRACIÓN AR (Dura 3 segundos)
            # =================================================================
            if mapeo_bloqueado:
                if tiempo_fase_asoc == 0: tiempo_fase_asoc = time.time()
                dt_asoc = time.time() - tiempo_fase_asoc
                
                # FASE 1 (0s - 1s): Mostrar puntos teóricos vs ArUcos reales
                if dt_asoc < 1.0:
                ...
                # FASE 2 (1s - 2s): Relacionar (Snap) el nodo teórico más cercano a cada ArUco físico
                elif dt_asoc < 2.0:
                    ...
                    if not asociacion_calculada:
                        for nodo in nodos_clave:
                            ...
                            if mejor_pt_w is not None: nodo['pt_aruco_w'] = mejor_pt_w # Enlaza la coordenada
                        asociacion_calculada = True
                    
                    for nodo in nodos_clave:
                        cv2.circle(lienzo_ar_dinamico, nodo['pt'], 6, (0, 0, 255, 255), -1)
                        if 'pt_aruco_w' in nodo:
                            cv2.line(lienzo_ar_dinamico, nodo['pt'], nodo['pt_aruco_w'], (0, 255, 255, 255), 2) # Dibuja la línea de emparejamiento
                    for pt_w in arucos_maze.values():
                        cv2.circle(lienzo_ar_dinamico, pt_w, 6, (255, 255, 0, 255), -1)

                # FASE 3 (2s - 3s): Sustituir las matemáticas por el mundo físico
                elif dt_asoc < 3.0:
                    txt_fase = "FASE 3: SNAP COMPLETADO"
                    if not asociacion_hecha:
                        for nodo in nodos_clave:
                            if 'pt_aruco_w' in nodo: nodo['pt'] = nodo['pt_aruco_w'] # El nodo teórico ahora tiene la coordenada real del ArUco.
                        
                        # Recalcula las pistas de entrada y salida rectas basadas en las posiciones iniciales puras.
                        v_ent_x = puntos[1][0] - puntos[0][0]
                        v_ent_y = puntos[1][1] - puntos[0][1]
                        norm_ent = max(1e-5, np.hypot(v_ent_x, v_ent_y))
                        pt_guia_entrada = (int(nodos_clave[0]['pt'][0] - (v_ent_x / norm_ent) * longitud_guia), int(nodos_clave[0]['pt'][1] - (v_ent_y / norm_ent) * longitud_guia))
                ...
                # FIN DE CALIBRACIÓN: Aplicación del Chaflán (Suavizado de esquinas)
                else:
                    if not reemplazo_hecho:
                        reemplazo_hecho = True
                        puntos_ruta = [pt_guia_entrada] + [n['pt'] for n in nodos_clave] + [pt_guia_salida]
                        nodos_suavizados = []
                        d_chaflan = 35 # Cuántos píxeles se corta la esquina  
                        ...
                        for i in range(len(nodos_clave)):
                            ...
                            # A cada giro le extraemos vectores de entrada y salida para recortar la punta.
                            if nodos_clave[i]['tipo'] in ['GIRO', 'GIRO_INICIO', 'FIN', 'INICIO_RECTO']:
                                ...
                                if n_in > 0 and n_out > 0:
                                    v_in /= n_in; v_out /= n_out # Normaliza
                                    
                                    # Asegura que el chaflán no sea más largo que la mitad de la recta
            ...
            # =================================================================
            # DIBUJO DE LA LÍNEA NEGRA (Renderizado AR)
            # =================================================================
            if reemplazo_hecho:
                grosor_linea_negra = 30
                txt_tiempo = f"Tiempo: {tiempo_transcurrido:.2f} s"

                # Dibuja la recta de entrada
                dibujar_entrada = False
                if indice_objetivo == 0 or (indice_objetivo == 1 and nodos_clave[0]['tipo'] == 'INICIO_RECTO'):
                ...
                # Dibuja la línea verde de Meta calculando el vector perpendicular al último tramo
                ...
                if norm_meta > 0:
                    v_meta /= norm_meta
                    v_perp_meta = np.array([-v_meta[1], v_meta[0]]) # Rotación 90 grados matemática
                    largo_meta = 60 
            ...
            # Proyección Holográfica 3D: Pega la capa plana deformada según la matriz de la cámara
            ...
            # =================================================================
            # LÓGICA DEL CEREBRO: SEGUIDOR DE LÍNEA DE 1 SENSOR
            # =================================================================
            if reemplazo_hecho:
                if not navegacion_iniciada:
                    ...
                    if cx_auto is not None and cy_auto is not None and vec_fwd_dir is not None:
                        mid_x = cx_auto; mid_y = cy_auto

                        if pt_sensor_central is not None:
                            # Aplanamos a coordenadas matemáticas 2D las posiciones vistas por la cámara
                            ...
                            # Función matemática pura: Mide la distancia más corta entre un punto y una recta finita
                            ...
                            # Evalúa toda la ruta para encontrar el punto exacto de la línea negra más cercano al sensor central
                            ...
                            # Si la distancia a la línea es menor a 12px, el sensor "pisa" la línea.
                            umbral_sensor = 12 
                            sensor_en_linea = dist_minima < umbral_sensor
                            
                            # Producto Cruz Geométrico: Compara la orientación del sensor respecto a la línea para saber si se salió por la Izquierda o Derecha.
                        ...
                        if not nodo_alcanzado:
                            # -------------------------------------------------------------
                            # SISTEMA ANTI-ATASCOS (PERRO GUARDÍAN)
                            # -------------------------------------------------------------
                            if cx_auto is not None and cy_auto is not None:
                                if posicion_anterior_auto is None:
                                    ...
                                    # Si el auto se mueve más de 10px, resetea el reloj de atasco.
                                    dist_movimiento = np.hypot(cx_auto - posicion_anterior_auto[0], cy_auto - posicion_anterior_auto[1])
                                    if dist_movimiento > 10:  
                                        posicion_anterior_auto = (cx_auto, cy_auto)
                                        tiempo_estatico = time.time()
                            
                            # Si activamos la reversa, la mantiene por 0.5 segundos y luego bloquea el canal (Radio Silence).
                            if estado_retroceso:
                            ...
                            # Si pasó el "periodo de gracia" de arranque (2s) y lleva atascado 2 segundos con buena señal WiFi:
                            ...
                            # -------------------------------------------------------------
                            # CONTROL DIRECCIONAL PURO
                            # -------------------------------------------------------------
                            else:
                                if sensor_en_linea:
                                    comando_ros2 = "ADELANTE"
                                else:
                                    # Si estamos tan cerca del último tramo (meta) que ya pisamos el final, forzamos ADELANTE.
                                    dist_a_meta, _ = dist_punto_segmento(pt_auto_w, puntos_ruta[-2], puntos_ruta[-1])
                                    if dist_a_meta < umbral_sensor:
                                         comando_ros2 = "ADELANTE"
                                    else:
                                        # Correcciones basadas en el Producto Cruz
                                        if cross_product < 0: comando_ros2 = "CORREGIR_DERECHA"
                                        else: comando_ros2 = "CORREGIR_IZQUIERDA"
                                    
                            # DETECCIÓN DE VICTORIA (Producto Punto a la Línea de Meta)
                            ...
                            if norm_exit > 0:
                                ...
                                # Si el vector del auto y el vector de la meta van en la misma dirección (cruzó la línea)
                                if np.dot(vec_auto_meta, v_exit_dir) > 0:
                                    comando_ros2 = "STOP"
                                    nodo_alcanzado = True
                                    cronometro_corriendo = False
                                    
                                    # GUARDA TODAS LAS MÉTRICAS EN TXT AL FINALIZAR
                    ...
                    # SI EL ARUCO NO SE VE EN LA CÁMARA (IGNORAR ESTE FOTOGRAMA)
                    else:
                        comando_ros2 = "IGNORAR"
                            
        # --- GENERACIÓN DE ETIQUETAS Y COLORES PARA LA UI ---
        ...
        # -----------------------------------------------------------------
        # EMISOR AL NODO ROS 2 (Con Filtro Anti-Spam)
        # -----------------------------------------------------------------
        if comando_ros2 != "IGNORAR":
        ...
        rclpy.spin_once(ros_node, timeout_sec=0.001) # Mantiene vivo el nodo ROS 2 en este fotograma
        
        # -----------------------------------------------------------------
        # DIBUJO DEL DASHBOARD NEGRO LATERAL
        # -----------------------------------------------------------------
        frame_ar_resized = cv2.resize(frame, (700, 540))
        vista_doble = np.zeros((540, 1000, 3), dtype=np.uint8)
        ...
        if txt_instrucciones: cv2.putText(vista_doble, txt_instrucciones, (x_base, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
        ...
        cv2.putText(vista_doble, txt_movimiento, (x_base, 230), cv2.FONT_HERSHEY_SIMPLEX, 0.45, color_cmd, 2)
        if txt_completado: cv2.putText(vista_doble, txt_completado, (x_base, 270), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

        if video_writer: video_writer.write(vista_doble)

        cv2.imshow("VALIDACION VECTOR (q: Salir)", vista_doble)
        
        # -----------------------------------------------------------------
        # LECTURA DE TECLADO (Controles)
        # -----------------------------------------------------------------
        ...
        if tecla == ord('q'): # SALIR
            ...
            break
        elif tecla == ord('i') and reemplazo_hecho: # INICIAR RECORRIDO Y VIDEO
            if not navegacion_iniciada:
                ...
                video_writer = cv2.VideoWriter(video_filename, cv2.VideoWriter_fourcc(*'mp4v'), 7.6, (1000, 540))
        elif tecla == ord('r'): # REINICIAR Y RESETEAR DATOS
